...
function splitStr(str: string, separator: string) {
	...
	return categoryArray.map((item, index) => (
		<span
			key={index}
			className="tag border border-solid border-white-2 px-3 rounded-xl"
		>
			{item}
		</span>
	));
							...
							<a className="w-full" target="_blank" href={project.link}>
								<figure className="project-img relative  aspect-video rounded-2xl overflow-hidden mb-4 before:absolute before:top-0 before:left-0 before:w-full before:h-full before:bg-opacity-0 z-[1] transition-1">
									{project.link !== "" ? (
										...
										<span
											style={{
												backgroundColor: "hsl(0, 43%, 51%)",
											}}
											className="text-[12px] text-white-1 absolute top-2 right-2 rounded-full px-3 py-1 flex justify-center items-center"
										>
											Awaiting deployment
										</span>
									)}
